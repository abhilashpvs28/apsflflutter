...
class BulkrenewScreen extends StatefulWidget {
  const BulkrenewScreen({super.key});

  @override
  State<BulkrenewScreen> createState() => _BulkrenewScreenState();
}

class _BulkrenewScreenState extends State<BulkrenewScreen> {
  // this is complted just submit api left
  bool? isChecked = true;
  ...
  Future<void> _bulkResume() async {
    ...
    var url = Uri.parse(Constants.baseUrl + Apiservice.bulk_renewal_submit);
    print("url $url");
  ...
  Future<void> _fetchData() async {
    ...
    var url = Uri.parse(Constants.baseUrl + Apiservice.bulk_renewal_list);
    print("url $url");
    final response = await http.post(url, headers: headers, body: body);
      ...
      if (isSelected) {
        if (_selectedItems.length >= 10) {
          toastification.show(
            ...
            backgroundColor: Color.fromARGB(255, 245, 241, 116),
