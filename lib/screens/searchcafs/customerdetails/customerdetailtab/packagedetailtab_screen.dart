...
class PackagedetailtabScreen extends StatefulWidget {
  final int? caf_id; // Add a field to store caf_id
  const PackagedetailtabScreen({super.key, this.caf_id});

  @override
  State<PackagedetailtabScreen> createState() => _PackagedetailtabScreenState();
}

class _PackagedetailtabScreenState extends State<PackagedetailtabScreen> {
  void initState() {
    super.initState();
  ...
  Future<void> _customer_details_names() async {
    ...
    try {
      var url = Uri.parse(Constants.baseUrl +
          Apiservice.customer_details_package +
